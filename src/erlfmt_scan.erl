...
-define(TRACK_NEWLINE_TOKEN(Token),
    Token =:= '(' orelse
        Token =:= '[' orelse Token =:= '{' orelse Token =:= '<<' orelse Token =:= '->'
).
...
%% TODO: annotate [, {, <<, (, -> with following newline info
%% to control user folding/expanding
...
split_tokens([{Atomic, Meta, Value} | Rest], Acc, CAcc) when ?IS_ATOMIC(Atomic) ->
    Token = {Atomic, atomic_anno(erl_anno:to_term(Meta)), Value},
