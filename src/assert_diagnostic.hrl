...
-define(FORMAT(T, Args), lists:flatten(io_lib:format(T, Args))).
...
-define(assertListEqual(A, B),
   ...
   case A of
        B -> ok;
        _ -> Errs = assert_diagnostic:check_length(A, B) ++
...
-define(assertMapsEqual(A, B),
   [?assert(maps:is_key(K, B), ?FORMAT("Item ~p => ~p not in result ~p", [K, V, B]))
            || {K, V} <- maps:to_list(A)],
   [?assert(maps:is_key(K, A), ?FORMAT("Item ~p => ~p not in expected ~p", [K, V, A]))
            || {K, V} <- maps:to_list(B)],
   [?assertEqual(maps:get(K, A), maps:get(K, B), ?FORMAT("For key ~p", [K]))
            || K <- maps:keys(A)]
).
