...
check_length(A, B) ->
   case length(B) - length(A) of
      0 -> [];
      D when D > 0 ->
         [?FORMAT("Actual list has ~p unexpected items: ~p", [D, lists:sublist(B, length(A)+1, D)])];
      D ->
         [?FORMAT("Actual list lacks ~p expected items: ~p", [-D, lists:sublist(A, length(B)+1, -D)])]
   end.
